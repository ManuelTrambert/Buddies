...
git.short(function (str) {
  results.git_commit = str;
  // console.log('config', config);
  fs.readFile('config.xml', function (err, data) {
    parser.parseString(data, function (err, result) {
      results.build = result.widget['$']['ios-CFBundleVersion'];
      results.version = result.widget['$'].version;
      // console.log(json);
    });
  });
});
      ...
      $.inject(
        gulp.src('app/main/constants/env-' + options.env + '.json'),
        {
          ...
          transform: function (filePath, file) {
            ...
            // console.log(json);
            json.git_commit = results.git_commit;
            json.version = results.version;
            json.build = results.build;
            if (json) {
